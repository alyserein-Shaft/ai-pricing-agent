...
export function SupplierPriceIntakeWorkspace({ projectId }: { projectId: string }) {
  ...
  return <section className="module-page supplier-price-intake-workspace" aria-label="Supplier quotation price review">
    ...
    {loading ? <p>Loading persisted supplier lines…</p> : error ? <p className="managed-document-error">{error}</p> : !supplierLines.length ? <div className="empty-state"><strong>No extracted supplier quotation lines</strong><p>Upload and confirm an XLS, XLSX, CSV or readable PDF Supplier Quotation, then start extraction.</p></div> : <div className="persistent-review-list">{supplierLines.map(row => <article key={row.id}><header><div><small>{row.supplier_name || "Supplier not identified"} · {row.quotation_reference || "No quotation reference"}</small><strong>{row.description || row.part_number || `Line ${row.row_number}`}</strong></div><span className={row.eligibility?.eligible ? "review-ready" : "review-pending"}>{row.eligibility?.eligible ? "ELIGIBLE FOR COSTING" : row.review_status}</span></header><dl><div><dt>Source</dt><dd>{row.sheet_name || "Sheet"} · row {row.row_number}</dd></div><div><dt>Manufacturer / model</dt><dd>{row.manufacturer || "Missing"} · {row.part_number || "Missing"}</dd></div><div><dt>Extracted price</dt><dd>{row.currency || "Missing currency"} {((row.net_price_minor ?? row.unit_price_minor ?? 0) / 100).toFixed(2)}</dd></div><div><dt>Validity</dt><dd>{row.issue_date || "Missing issue date"} → {row.valid_until || "Missing validity"}</dd></div><div><dt>Mapped product</dt><dd>{row.mapped_part_number || "UNMAPPED"}</dd></div><div><dt>Mapping basis</dt><dd>{row.mapping_basis || "Needs review"}</dd></div></dl>{row.eligibility?.blockers?.length ? <p className="managed-document-error">{row.eligibility.blockers.join(" · ")}</p> : null}<footer><small>Source document {row.document_id} · version {row.document_version_id}</small><button className="secondary-action" onClick={() => void openReview(row)}>Review line</button></footer></article>)}</div>}
