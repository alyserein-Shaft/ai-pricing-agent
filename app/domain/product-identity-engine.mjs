export const PRODUCT_IDENTITY_RULESET_VERSION="product-identity-v1.1";
export const PRODUCT_IDENTITY_RULES=[
 ...
 {id:"PI-050",priority:50,decision:"Attach only explicit relationships and source-backed discovery-only prices"},
 {id:"PI-060",priority:60,decision:"Do not silently resolve conflicting product units; preserve the conflict for review"},
];
...
export async function buildProductIdentityAnalysis({facts=[]}={}){
 ...
 for(const [identityKey,partFacts] of [...groups].sort(([a],[b])=>a.localeCompare(b))){
  ...
  const prices=related.filter(f=>f.fact_type==="Price"&&["Historical Catalogue Price","Supplier Quotation Price"].includes(f.attributes.priceType)&&f.attributes.partNumber===officialProductCode).map(f=>({knowledgeFactId:f.id,amount:String(f.attributes.amount),currency:f.attributes.currency||"Unknown",region:f.attributes.region||null,effectiveDate:f.attributes.effectiveDate||null,validity:f.attributes.validity||null,priceType:f.attributes.priceType,discoveryStatus:"Discovery Only",costingEligible:false,sourceLocation:f.sourceLocation}));
  ...
  const unitValues=[...new Set(related.filter(f=>f.fact_type==="Unit").map(f=>known(f.original_value)).filter(Boolean))];
  const conflictingUnits=unitValues.length>1;
  const resolvedUnit=conflictingUnits?null:(unitValues[0]||best(partFacts,"unit"));
