...
export const generateActions = (facts, workflow, project = {}) => {
  ...
  if (facts.missingPrices) add("missing-price", 78, "High", `Add prices for ${facts.missingPrices} item${facts.missingPrices === 1 ? "" : "s"}`, "Technically eligible items do not have current approved price evidence.", "Costing?status=missing-price", "Estimator", true);
...
export const calculateRisks = (facts, workflow, project = {}, now = new Date()) => {
  ...
  if (facts.missingPrices) add("Pricing coverage", facts.missingPrices === facts.boqItems ? "High" : "Medium", `${facts.missingPrices} BOQ item(s) missing eligible price`, "Cost and quotation values are incomplete.", "Costing?status=missing-price", "Estimator");
