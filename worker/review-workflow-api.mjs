...
export const handleReviewWorkflowApi = async (request, env) => {
  ...
  if (projectRoute) {
    ...
    if (operation === "queue" && request.method === "GET") { const status = url.searchParams.get("status"), priority = url.searchParams.get("priority"), search = url.searchParams.get("search"), clauses = [
      ...
      `(q.boq_item_id IS NULL OR EXISTS (SELECT 1 FROM ${currentBoqEvidenceFrom("active_b")} WHERE active_b.id=q.boq_item_id AND ${currentBoqItemPredicate("active_b")}))`,
    ...
    const currentItemExists = `(SELECT 1 FROM ${currentBoqEvidenceFrom("b")} WHERE b.id=q.boq_item_id AND ${currentBoqItemPredicate("b")})`;
    const items = (await env.DB.prepare(`SELECT q.* FROM review_queue_items q WHERE q.project_id=? AND q.deleted_at IS NULL AND (q.boq_item_id IS NULL OR EXISTS ${currentItemExists})`).bind(projectId).all()).results || [];
    const clarifications = (await env.DB.prepare(`SELECT c.* FROM review_clarifications c JOIN review_queue_items q ON q.id=c.review_item_id WHERE c.project_id=? AND q.deleted_at IS NULL AND (q.boq_item_id IS NULL OR EXISTS ${currentItemExists})`).bind(projectId).all()).results || [];
    const blocks = Number((await env.DB.prepare(`SELECT COUNT(*) count FROM safety_blocks sb JOIN safety_decisions d ON d.id=sb.safety_decision_id JOIN ${currentBoqEvidenceFrom("b")} ON b.id=d.boq_item_id WHERE d.project_id=? AND d.superseded_at IS NULL AND ${currentBoqItemPredicate("b")} AND sb.status='Open'`).bind(projectId).first())?.count || 0);
    const pricingPending = Number((await env.DB.prepare(`SELECT COUNT(*) count FROM ${currentBoqEvidenceFrom("b")} WHERE b.project_id=? AND ${currentBoqItemPredicate("b")} AND NOT EXISTS (SELECT 1 FROM project_dashboard_profiles p JOIN pricing_runs r ON r.scenario_id=p.selected_pricing_scenario_id AND r.superseded_at IS NULL JOIN pricing_lines l ON l.pricing_run_id=r.id AND l.boq_item_id=b.id WHERE p.project_id=b.project_id AND p.deleted_at IS NULL AND r.version_number=(SELECT MAX(r2.version_number) FROM pricing_runs r2 JOIN pricing_lines l2 ON l2.pricing_run_id=r2.id WHERE r2.scenario_id=p.selected_pricing_scenario_id AND r2.superseded_at IS NULL AND l2.boq_item_id=b.id) AND l.approval_ready=1 AND l.status NOT IN ('Invalid','Expired','Rejected'))`).bind(projectId).first())?.count || 0);
    const readiness = calculateQuotationReadiness({ reviewItems: items.map((x) => ({ ...x, reviewType: x.review_type, dueDate: x.due_date })), clarifications, safetyBlocks: blocks, pricingPending });
