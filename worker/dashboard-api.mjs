...
const organizationProjects = async (db, userId, organization, canSeeAllProjects, search = "", includeNonOperational = false) => db.prepare(`SELECT p.*, dp.*,
  COALESCE((SELECT pm.role FROM project_members pm WHERE pm.project_id=p.id AND pm.user_id=? AND pm.status='Active' AND pm.revoked_at IS NULL ORDER BY pm.granted_at DESC,pm.id DESC LIMIT 1), CASE WHEN p.owner_user_id=? THEN 'Project Manager' END) role,
  CASE WHEN p.operational_classification='Operational' THEN 0 ELSE 1 END is_test_fixture
  FROM projects p
  ...
  WHERE p.organization_id=? AND p.archived_at IS NULL AND dp.deleted_at IS NULL
    AND (?=1 OR p.operational_classification='Operational')
    AND (?=1 OR p.owner_user_id=? OR EXISTS (SELECT 1 FROM project_members visible_pm WHERE visible_pm.project_id=p.id AND visible_pm.user_id=? AND visible_pm.status='Active' AND visible_pm.revoked_at IS NULL))
    AND (?='' OR p.name LIKE ? OR COALESCE(dp.client,'') LIKE ? OR COALESCE(dp.tender_number,'') LIKE ?)
  ORDER BY p.updated_at DESC LIMIT 100`).bind(userId, userId, organization.id, includeNonOperational ? 1 : 0, canSeeAllProjects ? 1 : 0, userId, userId, search, `%${search}%`, `%${search}%`, `%${search}%`).all();

const uniqueProjects = (rows = []) => [...new Map(rows.map((project) => [project.id, project])).values()];
const uniqueActions = (projects = []) => [...new Map(projects.flatMap((project) => project.actions || []).map((action) => [`${action.projectId}:${action.id}`, action])).values()]
  .sort((a, b) => b.priority - a.priority || String(a.projectId).localeCompare(String(b.projectId)) || String(a.id).localeCompare(String(b.id)));
...
export const handleDashboardApi = async (request, env) => {
  ...
  if (url.pathname === "/api/projects" && request.method === "GET") {
    ...
    const rows = await organizationProjects(env.DB, user.id, active.organization, active.canSeeAllProjects, q, String(env.GOLDEN_E2E || "") === "1");
  ...
  if (url.pathname === "/api/projects" && request.method === "POST") {
    ...
    const projectId = id("project"), stamp = now(), operationalClassification = String(env.GOLDEN_E2E || "") === "1" ? "Internal Validation" : "Operational";
    await env.DB.batch([
      env.DB.prepare("INSERT INTO projects (id,name,owner_user_id,organization_id,system_domain,initial_status,operational_classification,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?)").bind(projectId, name, user.id, active.organization.id, systemDomain, initialStatus, operationalClassification, stamp, stamp),
      ...
      audit(env.DB, projectId, "Project Created", null, { name, organizationId: active.organization.id, systemDomain, initialStatus, operationalClassification }, "Created within the active server-derived organization", user, "Project Manager", request),
    ]);
    return json({ project: { id: projectId, name, organizationId: active.organization.id, ownerUserId: user.id, client: client || null, tenderNumber: reference || null, dueDate: dueDate || null, systemDomain, initialStatus, operationalClassification, isTestFixture: operationalClassification !== "Operational", role: "Project Manager" }, organization: active.organization }, 201);
  }
  if (url.pathname === "/api/dashboard/organization" && request.method === "GET") {
    const q = String(url.searchParams.get("q") || "").trim(), active = await resolveActiveOrganization(env.DB, user.id, String(url.searchParams.get("organizationId") || ""));
    if (active.error) return json({ error: active.error }, active.error.status);
    const includeNonOperational = String(env.GOLDEN_E2E || "") === "1", allRows = await organizationProjects(env.DB, user.id, active.organization, active.canSeeAllProjects, "", includeNonOperational), visibleRows = q ? await organizationProjects(env.DB, user.id, active.organization, active.canSeeAllProjects, q, includeNonOperational) : allRows;
    const allProjectRows = uniqueProjects(allRows.results || []), visibleProjectRows = q ? uniqueProjects(visibleRows.results || []) : allProjectRows;
    const allProjects = await hydrateProjects(env.DB, allProjectRows, user.id), projects = q ? await hydrateProjects(env.DB, visibleProjectRows, user.id) : allProjects, today = new Date().toISOString().slice(0, 10), soon = new Date(Date.now() + 7 * 86400000).toISOString().slice(0, 10), commercial = allProjects.filter((p) => !p.commercialRestricted), legacyCount = await unassignedLegacyCount(env.DB, user.id);
    ...
    return json({ modelVersion: DASHBOARD_MODEL_VERSION, scope: "organization", organization: active.organization, state: projects.length ? "Projects Available" : q ? "No Search Results" : "No Organization Projects", query: q, metrics, projects, unassignedLegacyProjects: { count: legacyCount, includedInMetrics: false }, actionQueue: uniqueActions(allProjects).slice(0, 50), updatedAt: now(), refreshAfterMs: allProjects.some((p) => p.facts.processing) ? 5000 : 15000 });
  }
