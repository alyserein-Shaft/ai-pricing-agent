...
OUTPUT_FILE = (
    OUTPUT_DIR
    / "Central_Kitchen_Structured_Cabling_Comparison_v3.xlsx"
)
...
def classify_final_data_item(
    ...
    if (
        ...
        or "testing" in text
        or "labeling" in text
        or "installation service" in text
        or "commissioning" in text
    ):
        return "Structured Cabling Service", "Service"

    if "rack installation" in text:
    ...
    if "patch panel 24p" in text or "24 port patch panel" in text:
...
def build_rfq_final_comparison(
    ...
    category_mapping = {
        ...
        "Outlet Jack": "Cat6A Jack",
        "Patch Panel Jack": "Cat6A Jack",
    ...
    for rfq_category, final_category in category_mapping.items():
