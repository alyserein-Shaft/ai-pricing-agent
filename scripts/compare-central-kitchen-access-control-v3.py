...
OUTPUT_FILE = OUTPUT_DIR / "Central_Kitchen_Access_Control_Comparison_v6.xlsx"
...
def classify_access_item(description: str) -> str:
    ...
    # Software and operator equipment
    if "hikcentral" in text and ("acs" in text or "access control" in text):
        return "Access Control Base License"

    if "license" in text or "software" in text:
        return "Access Control Base License"

    if "workstation" in text:
        return "ACS Workstation"

    if "monitor" in text or "display" in text:
        return "Monitor"

    # Controllers — specific rules before general controller scope
    if (
        "controller for four doors" in text
        or "four door controller" in text
        or "4 door controller" in text
        or "k2604" in text
    ):
        return "Four-door Controller"

    if (
        "controller for two doors" in text
        or "two door controller" in text
        or "2 door controller" in text
        or "k2602" in text
    ):
        return "Two-door Controller"

    if "access door system controller" in text:
        return "General Access Controller Scope"
    ...
    if (
        "push button" in text
        or "request to exit" in text
        or re.search(r"\\brex\\b", text)
    ):
    ...
    # Mounting bracket must be checked before magnetic lock
    if (
        "zl bracket" in text
        or "z l bracket" in text
        or "lock bracket" in text
        or "k4h250 lz" in text
    ):
        return "Lock Mounting Bracket"

    if (
        "door lock" in text
        or "magnetic lock" in text
        or "maglock" in text
    ):
    ...
    if "power supply" in text or re.search(r"\\bpsu\\b", text):
        return "Power Supply"

    if (
        "mifare card" in text
        or "access card" in text
        or "proximity card" in text
        or "credential" in text
    ):
        return "Access Card / Credential"
...
def build_relationships(
    ...
    category_mapping = {
        ...
        "Push Button / REX": ["Push Button / REX"],
        "Door Contact": ["Door Contact"],
        "Magnetic Lock": ["Magnetic Lock"],
        "Two-door Controller": ["Two-door Controller"],
        "Four-door Controller": ["Four-door Controller"],
        "General Access Controller Scope": [
            "Access Control Base License",
            "ACS Workstation",
            "Monitor",
            "Magnetic Lock",
    ...
    for source in source_records:
        ...
        target_categories = category_mapping.get(source_category, [])

        matches: list[dict[str, Any]] = []

        for target_category in target_categories:
            matches.extend(
                final_by_category.get(target_category, [])
            )

        if source_category == "General Access Controller Scope":
            relationship_type = "System Scope → Engineer Added Breakdown"
            review_status = "Mandatory Engineer Review"

        elif len(matches) == 1:
            source_qty = source.get("quantity")
            final_qty = matches[0].get("quantity")

            if (
                source_qty is not None
                and final_qty is not None
                and float(source_qty) == float(final_qty)
            ):
                relationship_type = "Direct Quantity Match"
                review_status = "Validated from Project Evidence"
            else:
                relationship_type = "Quantity Difference"
                review_status = "Needs Engineer Review"

        elif len(matches) > 1:
            relationship_type = "One-to-Many Breakdown"
            review_status = "Needs Engineer Review"

        else:
            relationship_type = "Not Found in Final Quotation"
            review_status = "Needs Engineer Review"
        ...
        relationships.append(
            {
                ...
                "quantity_relationship": (
                    "Source Qty = Final Qty"
                    if relationship_type == "Direct Quantity Match"
                    else ""
                ),
                "project_observation": (
                    "8 controlled-door points expanded to locks, brackets, "
                    "credentials, software and operator equipment."
                    if source_category == "General Access Controller Scope"
                    else ""
                ),
...
def build_validation(
    ...
    checks = [
        {
            "check": "Source Access Control items",
            "expected": 7,
        ...
        {
            "check": "Source Card Reader quantity",
            "expected": 8,
        ...
        {
            "check": "Final Card Reader quantity",
            "expected": 8,
            "actual": quantity_for_category(
                final_records,
                "final_category",
        ...
        {
            "check": "Source Push Button quantity",
            "expected": 8,
            "actual": quantity_for_category(
                source_records,
                "source_category",
        ...
        {
            "check": "Final Push Button quantity",
            "expected": 8,
            "actual": quantity_for_category(
                final_records,
                "final_category",
        ...
        {
            "check": "Source Door Contact quantity",
            "expected": 8,
            "actual": quantity_for_category(
                source_records,
                "source_category",
        ...
        {
            "check": "Final Door Contact quantity",
            "expected": 8,
            "actual": quantity_for_category(
                final_records,
                "final_category",
        ...
        {
            "check": "Source Magnetic Lock quantity",
            "expected": 8,
            "actual": quantity_for_category(
                source_records,
                "source_category",
        ...
        {
            "check": "Final Magnetic Lock quantity",
            "expected": 8,
            "actual": quantity_for_category(
                final_records,
                "final_category",
        ...
        {
            "check": "Source Two-door Controller quantity",
            "expected": 3,
            "actual": quantity_for_category(
                source_records,
                "source_category",
        ...
        {
            "check": "Final Two-door Controller quantity",
            "expected": 3,
            "actual": quantity_for_category(
                final_records,
                "final_category",
        ...
        {
            "check": "Source Four-door Controller quantity",
            "expected": 1,
            "actual": quantity_for_category(
                source_records,
                "source_category",
        ...
        {
            "check": "Final Four-door Controller quantity",
            "expected": 1,
            "actual": quantity_for_category(
                final_records,
                "final_category",
