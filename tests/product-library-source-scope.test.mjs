...
import { queryLibraryProducts, queryLibrarySources, visibleLibrarySource } from "../worker/product-price-library-api.mjs";
...
const fixture = () => {
  ...
  db.exec(`
    ...
    CREATE TABLE product_sources (id TEXT PRIMARY KEY, scope_type TEXT, project_id TEXT, metadata TEXT, created_at TEXT);
    ...
    INSERT INTO product_sources VALUES
      ('source-591','Global',NULL,'{"summary":{"productObservationsProcessed":591,"uniqueProductIdentities":591,"repeatedObservationsConsolidated":0,"priceObservationsDetected":590,"unresolvedRows":921,"explicitCurrencyPrices":0,"costingEligiblePrices":0}}','2026-08-13'),
      ('source-other','Global',NULL,'{}','2026-08-12'),
      ('source-private','Project','project-b','{}','2026-08-11');
  ...
  for (let index = 435; index <= 591; index += 1) {
    insertEvidence.run(`evidence-${index}`, `product-${index - 434}`, "source-591");
  }
...
test("591 persisted observations resolve to 434 source-scoped identities and 157 repeats", async () => {
  const { db } = fixture();
  const first = await queryLibraryProducts(db, { sourceId: "source-591", page: 1, pageSize: 200 });
  assert.equal(first.totalProducts, 434);
  ...
  const [source] = await queryLibrarySources(db);
  assert.equal(source.metadata.summary.productObservationsProcessed, 591);
  assert.equal(source.metadata.summary.uniqueProductIdentities, 434);
  assert.equal(source.metadata.summary.repeatedObservationsConsolidated, 157);
  assert.equal(source.metadata.summary.priceObservationsDetected, 590);
  assert.equal(source.metadata.summary.unresolvedRows, 921);
  assert.equal(source.metadata.summary.explicitCurrencyPrices, 0);
  assert.equal(source.metadata.summary.costingEligiblePrices, 0);
});

test("search and pagination remain source scoped while no-source browsing stays compatible", async () => {
  const { db } = fixture();
  const scoped = await queryLibraryProducts(db, { sourceId: "source-591", query: "PART-434", page: 1, pageSize: 50 });
  assert.equal(scoped.totalProducts, 1);
  assert.equal(scoped.products[0].requestedPartNumber, "PART-434");
  const pageThree = await queryLibraryProducts(db, { sourceId: "source-591", page: 3, pageSize: 200 });
  assert.equal(pageThree.products.length, 34);
  const global = await queryLibraryProducts(db, { page: 1, pageSize: 200 });
  assert.equal(global.totalProducts, 435);
